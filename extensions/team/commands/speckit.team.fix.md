...
Proceed only when the issue has label `state/working`.

If the issue does not have `state/working`, stop and explain that the issue is not in the correct state:

- If it has `state/new-issue`, tell the user the issue still needs review by the relevant people before fixing can start.
- If it has `state/accepted`, remind the user to claim the issue and switch the label to `state/working` before running `speckit.team.fix` again.
- For any other state or missing state label, tell the user which labels were found and that `state/working` is required.
