...
Close the release learning loop. This internal capability summarizes completed
feature and bugfix
...
- `work_ids=<comma-separated-work-slugs>` when tag discovery is unavailable
...
- `.specify/<category>/<work_id>/evidence/`;
- `.specify/<category>/<work_id>/spec.md`, `plan-and-task.md`, and `quickstart.md`;
- `.specify/bugfix/<work_id>/` for bugfix work;
...
```markdown
## <work_id or issue id>: <short symptom>
...
5. Promote durable knowledge:
   ...
   - feature architecture decisions to enterprise decision memory or migration
     playbook;
   - repeated AI mistakes to a later review or memory capability.
6. Update `archived-work.yml` with the status of each `work_id`:
   - `archived`;
   - `kept-active`;
   - `private-only`;
   - `superseded`;
   - `blocked`.
7. Use the configured memory capability for each lesson that should enter
   local, department, or enterprise memory.
...
- a work item has no stable issue, PR, tag, or `work_id`;
