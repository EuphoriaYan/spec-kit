...
2. Require an analysis-mode Permission Envelope. Verify that its repository,
   ...
   cover the proposed graph operation. Do not treat a human approval record as
   a runtime sandbox.
...
8. Hand the graph artifact to `speckit.team.plan-and-task`,
   `speckit.team.plan-and-task`, or `speckit.converge`
   (composite checks and evidence run inside converge when preset
   the active Team role requires it).
